// components/zonaintegritas/components/GratifikasiContent.jsx
"use client";

import React, { useState, useMemo, useEffect } from 'react';
import { gratifikasiData } from '../content/gratifikasi';
...
const GratifikasiContent = () => {
  ...
  const tahunOptions = useMemo(() => {
    const tahun = [...new Set(gratifikasiData.map(item => item.tahun))].sort((a, b) => b - a);
    return ['semua', ...tahun];
  }, []);

  // Get unique status untuk filter
  const statusOptions = useMemo(() => {
    const status = [...new Set(gratifikasiData.map(item => item.status))];
    return ['semua', ...status];
  }, []);

  // Filter data
  const filteredData = useMemo(() => {
    let result = gratifikasiData;
  ...
  const handleView = (item) => {
    alert(`Detail Laporan Gratifikasi:\n\nJudul: ${item.judul}\nStatus: ${item.status}\nKategori: ${item.kategori}\nTahun: ${item.tahun}\nDeskripsi: ${item.deskripsi}`);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-6 border border-purple-100">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-800">Sistem Pelaporan Gratifikasi</h2>
            <p className="text-sm text-gray-600">Pelaporan pemberian dan penerimaan hadiah/hibah</p>
          </div>
        </div>
        
        <div className="prose prose-sm max-w-none">
          <p className="text-gray-700">
            Sistem pelaporan gratifikasi sesuai dengan Peraturan Komisi Pemberantasan Korupsi 
            tentang Tata Cara Pelaporan Gratifikasi. Setiap pegawai wajib melaporkan penerimaan 
            gratifikasi dalam waktu 30 hari kerja sejak gratifikasi diterima.
          </p>
        </div>
      </div>
          ...
          <div className="flex-1">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 focus:outline-none"
              />
              {searchQuery && (
                <button
        ...
        {(searchQuery || filterStatus !== 'semua' || filterTahun !== 'semua') && (
          <div className="mt-3 pt-3 border-t border-gray-200">
            <div className="text-sm text-gray-600">
              <span className="font-semibold">{filteredData.length}</span> laporan ditemukan
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-purple-50 to-pink-50 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-800">Daftar Laporan Gratifikasi</h3>
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="px-3 py-2 text-sm font-medium bg-purple-50 text-purple-700 rounded-lg border border-purple-200 inline-block">
                          {item.tahun}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                ...
                <tr>
                  <td colSpan="4" className="px-6 py-12 text-center">
                    <div className="text-gray-400">
                      ...
                      <p className="font-medium text-gray-600 mb-2">Belum ada data laporan gratifikasi</p>
                      <p className="text-sm text-gray-500">
                        Data laporan gratifikasi akan ditampilkan di sini setelah tersedia
                      </p>
                    </div>
                  </td>
                </tr>
          ...
          <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-sm text-gray-600">
                ...
                <span className="font-semibold">{filteredData.length}</span> laporan
              </div>
...
export default GratifikasiContent;