// components/zonaintegritas/components/JadwalContent.jsx
"use client";

import React, { useState, useMemo, useEffect } from 'react';
import { jadwalData } from '../content/jadwal';
...
const JadwalContent = () => {
  ...
  const jenisOptions = useMemo(() => {
    const jenis = [...new Set(jadwalData.map(item => item.jenis))];
    return ['semua', ...jenis];
  }, []);

  // Get unique status untuk filter
  const statusOptions = useMemo(() => {
    const status = [...new Set(jadwalData.map(item => item.status))];
    return ['semua', ...status];
  }, []);
  ...
  const handleDownload = (item) => {
    alert(`Mengunduh jadwal: ${item.file}`);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-xl p-6 border border-indigo-100">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-800">Jadwal Pelayanan Publik</h2>
            <p className="text-sm text-gray-600">Informasi waktu dan tempat pelayanan</p>
          </div>
        </div>
        
        <div className="prose prose-sm max-w-none">
          <p className="text-gray-700">
            Jadwal pelayanan publik Balai Besar POM mencakup layanan administrasi, konsultasi, 
            pengaduan, pelatihan, dan sosialisasi kepada masyarakat sesuai dengan ketentuan 
          ...
          <div className="flex-1">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Cari jenis pelayanan, lokasi, atau judul..."
                className="w-full pl-10 pr-10 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:outline-none"
              />
              {searchQuery && (
                <button
        ...
        {(searchQuery || filterJenis !== 'semua' || filterStatus !== 'semua') && (
          <div className="mt-3 pt-3 border-t border-gray-200">
            <div className="text-sm text-gray-600">
              <span className="font-semibold">{filteredData.length}</span> jadwal ditemukan
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-indigo-50 to-blue-50 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-800">Daftar Jadwal Pelayanan</h3>
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex flex-col">
                          <span className="text-sm font-medium text-indigo-700">{item.tanggal}</span>
                          {item.waktu && (
                            <span className="text-xs text-gray-600">{item.waktu}</span>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex flex-col sm:flex-row gap-2">
                          ...
                          <button
                            ...
                            className="px-4 py-2 text-sm font-medium text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-lg border border-indigo-200 transition-colors flex items-center justify-center"
                          >
                            <svg className="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <tr>
                  <td colSpan="4" className="px-6 py-12 text-center">
                    <div className="text-gray-400">
                      ...
                      <p className="font-medium text-gray-600 mb-2">Belum ada data jadwal pelayanan</p>
                      <p className="text-sm text-gray-500">
                        Data jadwal pelayanan akan ditampilkan di sini setelah tersedia
                      </p>
                    </div>
                  </td>
                </tr>
          ...
          <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-sm text-gray-600">
                ...
                <span className="font-semibold">{filteredData.length}</span> jadwal
              </div>
